...
void loadList(DynArr *heap, FILE *filePtr)
{
	...
	printf(file, sizeof(file), "%%d, %%%d[^\n]",  TASK_DESC_SIZE);

	while (fscanf(filePtr, file, &t.priority, &t.description) == 2)
	{

		addHeap(heap, createTask(t.priority, t.description));
...
void printList(DynArr *heap)
{
	...
	while(sizeDynArr(temp) > 0)
	{
		t = getMinHeap(temp);
		printf("\n");
		printf("(%d, %s)", t.priority, t.description);
		printf("\n");
		removeMinHeap(temp);	
	}
	deleteDynArr(temp);
